...
const (
	deleteExistingLIDMappingQuery = `DELETE FROM waSocket_lid_map WHERE (lid<>$1 AND pn=$2)`
	putLIDMappingQuery            = `
		INSERT INTO waSocket_lid_map (lid, pn)
		VALUES ($1, $2)
		ON CONFLICT (lid) DO UPDATE SET pn=excluded.pn WHERE waSocket_lid_map.pn<>excluded.pn
	`
	getLIDForPNQuery       = `SELECT lid FROM waSocket_lid_map WHERE pn=$1`
	getPNForLIDQuery       = `SELECT pn FROM waSocket_lid_map WHERE lid=$1`
	getAllLIDMappingsQuery = `SELECT lid, pn FROM waSocket_lid_map`
)
